<template>
    <div>
        ...
        <form autocomplete="off" v-on:submit="register" v-if="!success">
			...
			<div class="form-group" v-bind:class="{ 'has-error': error && response.membertype }">
                <label for="name">Member Level</label>
                
				
				<select v-model="membertype" id="membertype" class="form-control" required >
				
					<option value = "admin" > admin</option>
					<option value = "user"> user </option>
					<option value = "coordinator"> coordinator</option>
				</select>
                <span class="help-block" v-if="error && response.membertype">{{ response.membertype }}</span>
            </div>
...
export default {
    ...
    methods: {
        register(event) {
            ...
            auth.register(this, this.name, this.email, this.password, this.membertype)
        }
    }
}
</script>